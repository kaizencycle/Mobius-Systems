# Kaizen-OS Handbook

<div style="text-align: center; margin: 2rem 0;">
  <p style="font-size: 1.2em; color: #FFD700; font-style: italic;">
    "We heal as we walk."
  </p>
  <p style="color: #F5E6D3;">
    Continuous Integrity Architecture
  </p>
</div>

---

## Welcome to Kaizen-OS

**Kaizen-OS** (formerly *Civic OS*) is a self-healing operating system for civilization: a framework that fuses DVA Kernel logic, Virtue Accords, and global integrity telemetry into a continuous improvement loop.

### Core Principles

- **改善 (Kaizen)** — Continuous Improvement  
  Small steps, daily practice, compounding forever.

- **召唤 (Summon)** — The Calling Forth  
  We recognize the spark in others and invite it by name.

- **金繕い (Kintsugi)** — Golden Repair  
  We honor the cracks; repair makes the story more beautiful.

---

## Quick Start

| I want to... | Go here |
|--------------|---------|
| **Understand the vision** | [README](../README.md) |
| **Start developing** | [Start Here](START_HERE.md) |
| **Deploy services** | [Deployment Guide](DEPLOYMENT_GUIDE.md) |
| **Learn architecture** | [Architecture Overview](architecture/overview.md) |
| **Understand economics** | [MIC Whitepaper](economics/GIC_WHITEPAPER_v1.md) |
| **Meet the agents** | [Founding Agents](agents/FOUNDING_AGENTS.md) |
| **Contribute** | [Contributing Guide](../CONTRIBUTING.md) |

---

## Documentation Structure

### 🏗️ Architecture
System design, specifications, and technical architecture documents.

- [System Overview](architecture/overview.md)
- [MIC Architecture Spec](architecture/GIC_Architecture_Spec_v1.md)
- [Founding Agents Stack](architecture/FOUNDING_AGENTS_SOVEREIGN_STACK.md)
- [Labs Architecture](LABS_MASTER_ARCHITECTURE.md)

### 🛡️ Governance
Governance protocols, policies, and frameworks.

- [Kintsugi Protocol](governance/kintsugi_protocol.md)
- [HR Framework](governance/hr/Kaizen_OS_HR_Framework.md)
- [AI Integrity Constitution](AI_INTEGRITY_CONSTITUTION.md)

### 💰 Economics
Economic models, MIC system, and UBI mechanisms.

- [MIC Whitepaper](economics/GIC_WHITEPAPER_v1.md)
- [Foundation Up Addendum](GIC_Foundation_Up_Economics_Addendum.md)
- [UBI Mechanism](whitepapers/UBI_Mechanism_v2.0.md)

### 🤖 Sentinels & Agents
AI agents, sentinels, and companion systems.

- [Founding Agents](agents/FOUNDING_AGENTS.md)
- [Companions](companions/README.md)
- [Sentinels](sentinels/uriel.md)

### 🔧 Development
Developer guides, APIs, and technical documentation.

- [Frontend Development](FRONTEND_DEVELOPMENT.md)
- [Deployment Guide](DEPLOYMENT_GUIDE.md)
- [Vercel Deployment](deployment/VERCEL_DEPLOYMENT_RUNBOOK.md)
- [API Integration](API_INTEGRATION.md)

### 🧪 Labs
Lab proof systems and implementations.

- [Lab 4: E.O.M.M. Reflections](archive/completed/labs/lab4-proof/README-COMPLETE.md)
- [Lab 6: Citizen Shield](archive/completed/labs/lab6-proof/README-COMPLETE.md)
- [Lab 7: OAA Hub](lab7/004-evolution-language.md)

### 📊 Integrity
Mobius Integrity Index (GI) system and drift control.

- [GI Formula](ledger/gi-formula.md)
- [Drift Control](drift/README.md)
- [AI Integrity](AI_INTEGRITY_CONSTITUTION.md)

### 📚 Philosophy
Core philosophical principles and theorems.

- [Kaizen Theorems](kaizen_theorems.md)
- [Triad of Healing](manifesto/triad_of_healing.md)
- [Opening Invocation](rituals/opening_invocation.md)

---

## System Status

<div class="kaizen-badge">GI: ≥ 0.95</div>
<div class="kaizen-badge">Status: Active</div>
<div class="kaizen-badge">Version: 0.2</div>

---

## Getting Help

- **Discord**: Join our community (link in main README)
- **Issues**: [GitHub Issues](https://github.com/kaizencycle/Mobius-Systems/issues)
- **Discussions**: [GitHub Discussions](https://github.com/kaizencycle/Mobius-Systems/discussions)

---

## License

This documentation is licensed under [CC-BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/).

---

<div class="kaizen-divider"></div>

*Last updated: {{ git_revision_date_localized }}*
